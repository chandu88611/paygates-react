...
function WL_DynamicCart() {
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      try {
        const res = await axios.post("https://finance.ftxtradingsoftware.com/api/pixel-fire", {
  ...
  const checkout = async () => {
    ...
    try {
      const res = await axios.post(
        "https://parent.globalcrmsoftware.com/api/worldline-api",
        {
          data:data,
          type:"child"
        }
     ...
     if(res.data.status){
    worldline(res.data.wordLineFormData)
     }
    setLoading(false);
    } catch (error) {
      console.log(error);
  ...
  const worldline=(res)=>{
    ...
    var reqJson = {
      ...
      "consumerData": {
          ...
          "token": res.hash,
          "returnUrl": `https://checkout.globalcrmsoftware.com/payment-status/wl/${res?.orderNumber}/${res?.txnId}`,    //merchant response page URL
          "responseHandler": handleResponse,
          "paymentMode": "all",
          "merchantLogoUrl": "https://checkout.globalpbxsoftware.com/ig-logo.jpg",  //provided merchant logo will be displayed
          "merchantId":res?.marchantId,
          "currency": res?.currencycode,
          "consumerId":res?.consumerId,
          "consumerMobileNo": res?.mobileNumber,
          "consumerEmailId": res?.email,
          "txnId":res?.txnId,   //Unique merchant transaction ID
          "items": [{
              "itemId": res?.schemecode,
              "amount": res?.amount,
              "comAmt": "0"
          }],
          "customStyle": {
  ...
  return (
    <div className="my-2">
      {data && !error ? (
        <div className=" mt-4">
          <div class="  w-full sm:w-[80vw] md:w-[90vw] lg:w-[84vw] 2xl:w-[60vw] mx-auto mt-2 md:px-1  p-2 shadow rounded">
            <div class="row">
...
<p className='font-semibold'>{ipData?.address?.city}</p>
                  ...
                  <div>
                    ...
                    {data?.invoice?.items?.length > 0 ? (
                      ...
                      //             {data1?.total}
                      //           </span>{" "}
                      //           <span class="text-muted !text-[12px] !md:text-[14px]">
                      //             {" "}
                      //             {data1?.tax_val + "%"}{" "}
                      //           </span>
                      //         </div>
                    ...
                    <div class="flex  items-center mt-3 mb-4  ">
                      <div class="w-full pr-4">
                        <div className="flex items-center gap-4  ">
                          ...
                          />
                          <button
                            className="bg-purple-100 px-3 py-2 rounded text-purple-600 hover:bg-purple-500 hover:text-white text-[15px] "
                            onClick={() => {
                              if (promo) {
      ...
      {loading && <Proccesing />}
      <form class="initiatePaymentForm opacity-0 h-0 overflow-auto" id="payu-form" action="https://secure.payu.in/_payment" method="post" name="payuForm"  ref={formRef} >
            ...
            <input type="text" class="form-control" id="currency" placeholder="Enter currency" name="amount" value={formData?.amount}/>
            <input type="text" class="form-control" id="currency" placeholder="Enter currency" name="firstname" value={formData?.firstname}/>
            <input type="text" class="form-control" id="currency" placeholder="Enter currency" name="lastname" value={formData?.lastname}/>
            <input type="text" class="form-control" id="amount" placeholder="Enter amount" name="email"  value={formData?.email}/>
            <input type="email" class="form-control" id="buyerEmail" placeholder="Enter email" name="productinfo" value={formData?.productinfo}/>
            ...
            <input type="hidden" name="zipcode" id="zipcode" value={formData?.zipcode}/>
            <input type="hidden" name="udf1" id="udf1" value={formData?.udf1}/>
            <input type="hidden" name="udf2" id="udf2" value={formData?.udf2}/>
            <input type="hidden" name="udf2" id="udf2" value={formData?.udf2}/>
            <input type="hidden" name="udf3" id="udf3" value={formData?.udf3}/>
            <input type="hidden" name="udf4" id="udf4" value={formData?.udf4}/>
            <input type="hidden" name="udf5" id="udf5" value={formData?.udf5}/>
            <input type="hidden" name="pg" id="pg" value={formData?.pg}/>
