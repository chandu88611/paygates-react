...
function Binance_DynamicCart_infinity() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      try {        
        const res = await axios.post("https://finance.infinitysoftware.asia/api/pixel-fire",{paymentLinkId:params?.id});
        if(res){
          setLoading(false)
        }
  ...
  const checkout = async () => {
    ...
    try {
      const res = await axios.post(
        "https://parent.globalcrmsoftware.com/api/binance-api",
        {
...
export default Binance_DynamicCart_infinity;
