...
function FloxyPay_DynamicCart_auto() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      try {        
        const res = await axios.post("https://finance.autotradesoftware.ae/api/pixel-fire",{paymentLinkId:params?.id});
        if(res){
          setLoading(false)
        }
  ...
  const checkout = async () => {
    ...
    try {
      const res = await axios.post("https://parent.globalcrmsoftware.com/api/floxypay-api",{
...
export default FloxyPay_DynamicCart_auto;
